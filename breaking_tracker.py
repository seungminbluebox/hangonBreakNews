...
def fetch_latest_headlines():
    headlines = []
    # 1. 기준 시간 설정 (시차 지연 및 언론사 RSS 반영 지연 대비: 2시간(120분)으로 여유 있게 설정)
    # 진짜 필터링은 DB 중복 체크 + AI 문맥 파악이 담당하므로 시간은 넉넉하게 잡는 것이 안전함.
    now_utc = datetime.now(timezone.utc)
    time_limit_utc = now_utc - timedelta(minutes=120)
...
def filter_breaking_news(headlines, recent_news_list):
    ...
    instructions = f"""당신은 글로벌 경제 및 증시 트렌드를 발빠르게 전달하는 수석 에디터입니다.
...
1. **필터링 대상 (무조건 Skip - 배열에서 완전히 제외)**: 
    - "증시 마감 요약", "주간 동향", "오늘의 시장 정리(Wrap-up)", "경제 지표 프리뷰(Preview)"
    - "테슬라 주가가 급락한 3가지 이유", "향후 전망과 분석(Opinion/Analysis)", "전문가 칼럼" 등.
    - **경제/증시/지정학과 전혀 무관한 가십성 뉴스**: 지역 축제, 동물, 스포츠, 연애, 엔터테인먼트, 단순 개인 사건사고 등. (반드시 0점 처리 및 제외)
...
- 중요도(importance_score): 글로벌 경제/증시/지정학적 파급력에 따라 1~10점으로 부여하되, 가치 없는 기사는 0점을 주고 제외하세요. **주가나 시장에 즉각적인 영향을 줄 수 있는 의미 있는 지표 발표, 실적, 정책, 중대 사건일 때에만 7점 이상**을 주어 엄격하게 통과시키세요. 일반적인 해설이나 약한 단신은 6점 이하로 배정하세요.
...
- content: 수치나 핵심 팩트를 포함하여 1~2문장으로 압축. (반드시 '~입니다' 체 사용 및 시장 영향가 포함)
...
- **보고 형식 및 시장 영향**: 모든 문장은 정중한 **'~입니다'**체로 작성해 주세요. 또한, 해당 뉴스가 향후 **시장에 어떤 영향을 미칠지(호재/악재/변동성 등) 혹은 관전 포인트**를 한 문장으로 포함해 주세요.
...
[
    ...
    "content": "수치가 포함된 팩트 요약과 시장 영향력을 정중한 '~입니다' 체로 설명 (150자 이내)",
...
위의 [최근 보도된 속보] 리스트를 반드시 읽으세요. 이미 보도된 사건과 '동일한 사실(원인/결과)'을 다루는 기사라면, 다른 언론사에서 다르게 썼더라도 무조건 배열에서 완전히 삭제(제외)하세요. 또한 스포츠, 동물, 가십 등 무관한 기사도 제외하십시오.
"""
